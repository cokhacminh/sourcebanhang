								...
								<table class="table table-bordered table-striped mb-none" id="datatable-default">
									...
									<tbody>
										...
										while($do = mysql_fetch_array($sql))
										{
											...
											$ten = $do['ten'];
											$leader = $do['leader'];
											$leadername = getname($leader);
											$mavandon = $do['mavandon'];
											$idapi = $do['idapi'];
											$listnhanvien = "";
											$a = mysql_query("select ten from api where id = '{$idapi}'");
											$b = mysql_fetch_array($a);
											$c = mysql_query("select id,fullname from user where team_id ='{$id}'");
											while($d = mysql_fetch_array($c))
											{
												
												if($d['id'] != $leader)
												$listnhanvien.="<font size='4'><b>{$d['fullname']}<a onclick=\"kickout({$d['id']})\"> <img src='../images/delete.png' width='20px' /></a></b></font><br />";
											}
											$tenapi = "API ".$b['ten'];
											$hotline = $do['hotline'];
											echo "
											...
											<td style=\"vertical-align: middle;text-align:center\">{$leadername}</td>
											<td style=\"vertical-align: middle;text-align:center\">{$mavandon}</td>
											
											<td style=\"vertical-align: middle;text-align:center\">{$tenapi}</td>
											<td style=\"vertical-align: middle;text-align:center\">{$hotline}</td>
											<td style=\"vertical-align: middle;text-align:center\">{$listnhanvien}</td>
											<td style=\"vertical-align: middle;text-align:center\"><a class=\"mb-xs mt-xs mr-xs btn btn-danger\" onclick=\"xoateam({$id})\"><i class=\"fa fa-user-times\"></i> XÓA TEAM</a></td>
									...
									<div id="modalForm_addmember" class="modal-block modal-block-primary mfp-hide" style="font-size: 18px;">
										<section class="panel">
											...
											<div class="panel-body">
													...
													<div class="form-group mt-lg">
														<?php
															$sql_nv_empty_team = mysql_query("select id,fullname from user where team_id='0' and id!='1'");
...
$("form#add_team").submit(function(){
    ...
    $.ajax({
        url: '../ajax/team.php?dosomething=add_team',
        type: 'POST',
        data: formData,
        async: false,
...
$("form#form_editteam").submit(function(){
    ...
    $.ajax({
        url: '../ajax/team.php?dosomething=edit_team',
        type: 'POST',
        data: formData,
        async: false,
        success: function (data) {
...
$("form#add_user").submit(function(){
    ...
    $.ajax({
        url: '../ajax/team.php?dosomething=add_user',
        type: 'POST',
        data: formData,
        async: false,
        success: function (data) {
