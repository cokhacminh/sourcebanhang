...
if(isset($_POST['tendvgh']))
{
	
	$tendvgh = $_POST['tendvgh'];
	$shortname = CreatShortName($tendvgh);
	if(!isset($_POST['ghichu']))$ghichu = "";
	else $ghichu = $_POST['ghichu'];
	// Nếu người dùng có chọn file để upload
  if($_FILES['logodvgh']['name'] != NULL)
  	{ // Đã chọn file
        // Tiến hành code upload file
        if($_FILES['logodvgh']['type'] == "image/jpeg"
        || $_FILES['logodvgh']['type'] == "image/png"
        || $_FILES['logodvgh']['type'] == "image/jpg"
        || $_FILES['logodvgh']['type'] == "image/gif")
        {
        // là file ảnh
        // Tiến hành code upload    
                ...
                $tmp_name = $_FILES['logodvgh']['tmp_name'];
                $name = $_FILES['logodvgh']['name'];
                $type = $_FILES['logodvgh']['type']; 
                $size = $_FILES['logodvgh']['size']; 
                // Upload file
    ...
    $result = mysql_query("insert into donvigiaohang (ten,logo,ghichu) values ('{$tendvgh}','{$hinhanh}','{$ghichu}')");
    if($result)echo "
<script>
swal({
  title: 'HOÀN TẤT',
  text: 'Đã thêm đơn vị giao hàng :  {$tendvgh} ! ',
  type: 'success',
