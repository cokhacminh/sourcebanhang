...
<section class="panel">
							...
							<div class="panel-body" style="font-size: 16px;">
								<div class="row">
									<div class="col-sm-6">
										<div class="mb-md">
											<a class="modal-with-form btn btn-primary" href="#modalForm_themnhom">Thêm Nhóm Nhân Viên <i class="fa fa-user-plus"></i></a>
										</div>
									</div>
								</div>
								<table class="table table-bordered table-striped mb-none" id="datatable-default">
									...
									<tbody>
										<?php
										$sql = mysql_query("select * from usergroup where id !='1'");
										while($do = mysql_fetch_array($sql))
										{
											$id = $do['id'];
											$ten = $do['ten'];
											$banhang = ($do['banhang'] == '1') ? "Có" : "Không";
											$quanlynhanvien = ($do['quanlynhanvien'] == '1') ? "Có" : "Không";
											$xuatkho = ($do['xuatkho'] == '1') ? "Có" : "Không";
											$nhapkho = ($do['nhapkho'] == '1') ? "Có" : "Không";
											echo "
											<tr class=\"gradeX\">
											<td style=\"vertical-align: middle;text-align:center\">{$id}</td>
											<td style=\"vertical-align: middle;text-align:center\">{$ten}</td>
											<td style=\"vertical-align: middle;text-align:center\">{$banhang}</td>
											<td style=\"vertical-align: middle;text-align:center\">{$quanlynhanvien}</td>
											<td style=\"vertical-align: middle;text-align:center\">{$nhapkho}</td>
											<td style=\"vertical-align: middle;text-align:center\">{$xuatkho}</td>
											<td style=\"vertical-align: middle;text-align:center\"><a href=\"#modalForm_suanhom\" class=\"hvr-float modal-with-form mb-xs mt-xs mr-xs btn btn-success\" title=\"Chỉnh sửa sản phẩm này\" onclick=\"suanhom({$id})\"><i class=\"fa fa-pencil-square-o\"></i> CHỈNH SỬA</a> <a class=\"mb-xs mt-xs mr-xs btn btn-danger\" title=\"Xóa tài khoản này\" onclick=\"xoanhom({$id})\"><i class=\"fa fa-user-times\"></i> XÓA</a></td>
										</tr>
											";
						...
						<!--Thêm Tài Khoản-->
									<div id="modalForm_themnhom" class="modal-block modal-block-primary mfp-hide" style="font-size: 18px;">
										<section class="panel">
											<header class="panel-heading">
												<h2 class="panel-title">Thêm nhóm nhân viên</h2>
											</header>
													...
													<div class="form-group">
														...
														<div class="col-sm-9">
															...
															<div class="checkbox-custom checkbox-danger">
																<input type="checkbox" id="checkboxExample4" name="xuatkho" value="1">
						...
						<!--Sửa tài khoản -->
						<div id="modalForm_suanhom" class="modal-block modal-block-primary mfp-hide" style="font-size: 18px;">
										<section class="panel">
											<header class="panel-heading">
												<h2 class="panel-title">Chỉnh Sửa Nhóm Nhân Viên</h2>
											</header>
											<form id="suanhom" action="" method="post" enctype="multipart/form-data">
											<div class="panel-body">
...
$("form#themnhom").submit(function(){
    ...
    $.ajax({
        url: 'ajax_usersgroup.php?dosomething=add_user',
        type: 'POST',
        data: formData,
        async: false,
...
//Sửa tài khoản
$("form#suanhom").submit(function(){

    var formData = new FormData(this);

    $.ajax({
        url: 'ajax_usersgroup.php?dosomething=edit_user',
        type: 'POST',
        data: formData,
        async: false,
        success: function (data) {
