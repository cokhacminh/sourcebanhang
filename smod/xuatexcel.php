...
foreach ($data as $index => $item) 
{
    ...
    // C1, C2, C3, ...
    $PHPExcel->getActiveSheet()->setCellValue('E' . $rowNumber, $item[5]);
        // C1, C2, C3, ...
    $PHPExcel->getActiveSheet()->setCellValue('F' . $rowNumber, $item[9]);
        // C1, C2, C3, ...
    $PHPExcel->getActiveSheet()->setCellValue('G' . $rowNumber, $item[7]);
        // C1, C2, C3, ...
    $PHPExcel->getActiveSheet()->setCellValue('H' . $rowNumber, $donhang);
    $PHPExcel->getActiveSheet()->setCellValue('I' . $rowNumber, $item[11]);
        // C1, C2, C3, ...
    $PHPExcel->getActiveSheet()->setCellValue('J' . $rowNumber, $item[12]);
	
     $PHPExcel->getActiveSheet()->setCellValue('K' . $rowNumber, $guihang);
	 $PHPExcel->getActiveSheet()->setCellValue('L' . $rowNumber, $item[17]);
